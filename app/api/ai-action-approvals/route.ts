...
import { db } from "@/db";
import { aiEmployeeActionApprovals } from "@/db/schema";
...
export async function GET() {
  ...
  const approvals = await db
    ...
    .where(eq(aiEmployeeActionApprovals.businessId, membership.businessId))
    .orderBy(desc(aiEmployeeActionApprovals.createdAt));

  return NextResponse.json({
    success: true,
    approvals: approvals.map((row) => ({ ...row, payload: JSON.parse(row.payload) })),
  });
}
