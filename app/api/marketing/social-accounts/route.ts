...
export async function GET() { const access = await requireMarketingAccess("view"); if (!access.ok) return NextResponse.json({ error: access.error }, { status: access.status }); const accounts = await getMarketingChannelAccounts(access.businessId); return NextResponse.json({ accounts, providers, readiness: marketingChannelReadiness(accounts) }); }
