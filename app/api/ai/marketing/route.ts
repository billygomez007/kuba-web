...
export async function POST(request: Request) {
  try {
    ...
    if (!isEmployeeTypeEntitled(await getBusinessEntitlements(business.id), "marketing")) {
      return NextResponse.json(
        {
          error: "Kuba Marketing requires the Pro plan or higher.",
    ...
    if (!employee) {
      return NextResponse.json(
        { error: "This Marketing employee is not active for your business." },
        { status: 404 },
      );
    }
    ...
    // Scoped to this business, so Marketing can never retrieve another
...
metrics, customers, leads, campaigns, or performance data. If information
is unavailable, say so clearly.
...
MARKETING RESPONSE RULES

- Draft content directly in your response; you have no tool that publishes or sends anything.
- Always label drafts as drafts requiring human review.
...
`;
    ...
    const result = await withAIUsageLogging(
      ...
      () =>
        kubaMarketingAgent.generate(prompt, {
          memory: {
            resource: session.user.id,
            thread: `marketing-${employee.id}`,
          },
    ...
    console.error("Kuba Marketing error:", error, { category: classifyAIProviderError(error) });

    return NextResponse.json({ error: "Kuba Marketing was unable to respond." }, { status: 500 });
  }
}
