...
export async function POST(request: Request) {
  try {
    ...
    if (!isEmployeeTypeEntitled(await getBusinessEntitlements(business.id), "finance")) {
      return NextResponse.json(
        {
          error: "Kuba Finance requires the Pro plan or higher.",
    ...
    if (!employee) {
      return NextResponse.json(
        { error: "This Finance employee is not active for your business." },
        { status: 404 },
      );
    }
    ...
    // Scoped to this business, so Finance can never retrieve another
    ...
    try {
      ...
      console.error("Finance knowledge search error:", error);
    }
...
Only use information belonging to this business. Never invent a financial
figure, forecast input, or trend. If information is unavailable, say so
clearly.
...
- You must never claim to move money, execute a transaction, or approve a purchase. Label forecasts and scenarios explicitly as estimates.
...
`;
    ...
    const result = await withAIUsageLogging(
      ...
      () =>
        kubaFinanceAgent.generate(prompt, {
          memory: {
            resource: session.user.id,
            thread: `finance-${employee.id}`,
          },
    ...
    console.error("Kuba Finance error:", error, { category: classifyAIProviderError(error) });

    return NextResponse.json({ error: "Kuba Finance was unable to respond." }, { status: 500 });
  }
}
