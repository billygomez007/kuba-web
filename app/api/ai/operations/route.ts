...
export async function POST(request: Request) {
  try {
    ...
    if (!isEmployeeTypeEntitled(await getBusinessEntitlements(business.id), "operations")) {
      return NextResponse.json(
        {
          error: "Kuba Operations requires the Pro plan or higher.",
    ...
    const employeeResult = await db
      ...
      .where(
        and(
          ...
          eq(aiEmployees.type, "operations"),
    ...
    if (!employee) {
      return NextResponse.json(
        { error: "This Operations employee is not active for your business." },
        { status: 404 },
      );
    }
    ...
    // Scoped to this business, so Operations can never retrieve another
    ...
    try {
      ...
      console.error("Operations knowledge search error:", error);
    }
...
Only use information belonging to this business. Never invent a task,
appointment, or automation result. If information is unavailable, say so
clearly.
...
OPERATIONS RESPONSE RULES

- Only report task, appointment, and automation data returned by your tools.
...
`;
    ...
    const result = await withAIUsageLogging(
      ...
      () =>
        kubaOperationsAgent.generate(prompt, {
          memory: {
            resource: session.user.id,
            thread: `operations-${employee.id}`,
          },
    ...
    console.error("Kuba Operations error:", error, { category: classifyAIProviderError(error) });

    return NextResponse.json({ error: "Kuba Operations was unable to respond." }, { status: 500 });
  }
}
