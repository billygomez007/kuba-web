...
export async function POST(request: Request) {
  try {
    ...
    if (!isEmployeeTypeEntitled(await getBusinessEntitlements(business.id), "hr")) {
      return NextResponse.json(
        {
          error: "Kuba HR requires the Pro plan or higher.",
    ...
    if (!employee) {
      return NextResponse.json(
        { error: "This HR employee is not active for your business." },
        { status: 404 },
      );
    }
    ...
    // Scoped to this business, so HR can never retrieve another
    ...
    try {
      ...
      console.error("HR knowledge search error:", error);
    }
...
Only use information belonging to this business. Never invent an employee,
leave balance, department, or HR policy. If information is unavailable, say
so clearly.
...
- You must never make or imply a hiring, firing, discipline, or compensation decision, and must never use protected-characteristic data.
- Only report headcount, department, and leave figures returned by your tools.
...
`;
    ...
    const result = await withAIUsageLogging(
      ...
      () =>
        kubaHrAgent.generate(prompt, {
          memory: {
            resource: session.user.id,
            thread: `hr-${employee.id}`,
          },
    ...
    console.error("Kuba HR error:", error, { category: classifyAIProviderError(error) });

    return NextResponse.json({ error: "Kuba HR was unable to respond." }, { status: 500 });
  }
}
