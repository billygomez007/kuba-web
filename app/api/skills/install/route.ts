...
export async function POST(request: Request) {
  try {
    ...
    const employeeId = validId(body.employeeId, "Employee ID");
    ...
    return authorizationErrorResponse(error) ?? validationErrorResponse(error) ?? NextResponse.json({ error: "Unable to install skill." }, { status: 500 });
  }
}
