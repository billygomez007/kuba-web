...
export default function CustomerOperationsPage() {
  ...
  useEffect(() => { void Promise.all([fetch("/api/inbox/workspace", { cache: "no-store" }), fetch("/api/customers", { cache: "no-store" }), fetch("/api/appointments?upcoming=true", { cache: "no-store" }), fetch("/api/tickets?status=open", { cache: "no-store" })]).then(async ([workspaceResponse, customerResponse, appointmentResponse, ticketResponse]) => {
    const workspace = await workspaceResponse.json(); const customerData = await customerResponse.json(); const appointmentData = await appointmentResponse.json(); const ticketData = await ticketResponse.json();
    ...
    setOperations(workspace); setCustomers(customerData.customers?.length || 0); setAppointmentCount(appointmentResponse.ok ? appointmentData.appointments?.length || 0 : null); setTicketCount(ticketResponse.ok ? ticketData.tickets?.length || 0 : null);
  ...
  const cards = [
    ...
    ["Pending handoffs", operations?.handoffs.filter((item) => item.status === "pending").length ?? "…", "/dashboard/handoffs"], ["Conversations today", operations?.metrics.conversationsToday ?? "…", "/dashboard/inbox"], ["Upcoming appointments", appointmentCount ?? "…", "/dashboard/appointments"], ["Open tickets", ticketCount ?? "…", "/dashboard/tickets"],
  ] as const;
