    ...
    <main className="min-h-screen bg-[#07070A] px-4 py-8 text-white sm:px-6 lg:px-8 lg:py-12">
      <div className="mx-auto max-w-[1500px]">
        ...
        <Section eyebrow="AI employee understanding" title="Knowledge access by employee">
          <div className="grid gap-4 md:grid-cols-2">{brain.employees.length ? brain.employees.map((employee) => <div key={employee.id} className="rounded-2xl border border-white/10 bg-white/[0.025] p-5"><div className="flex items-start justify-between gap-3"><div><h3 className="font-bold">{employee.name}</h3><p className="mt-1 text-xs capitalize text-white/35">{employee.type.replaceAll("-", " ")}</p></div><Status status={employee.status} /></div><p className="mt-5 text-xs uppercase tracking-wider text-white/30">Access</p><div className="mt-3 flex flex-wrap gap-2">{employee.access.length || employee.dedicatedSources.length ? [...new Set([...employee.access, ...employee.dedicatedSources])].map((source) => <span key={source} className="rounded-lg border border-cyan-300/15 bg-cyan-300/[0.06] px-2.5 py-1.5 text-xs text-cyan-200">✓ {source}</span>) : <span className="text-sm text-white/35">Shared business profile only</span>}</div></div>) : <Empty message="Activate an AI employee to see knowledge access." />}</div>
        </Section>
