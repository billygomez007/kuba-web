...
const capabilityLabels: Partial<Record<Capability, string>> = {
  ...
  "customer_ops.ai_assist": "AI-assisted appointments & tickets",
          ...
          {planOrder.map((planId) => {
            ...
            return (
              <article key={planId} className={`relative flex flex-col rounded-2xl border p-6 ${copy.recommended ? "border-cyan-300/50 bg-cyan-300/[0.08] shadow-[0_20px_80px_rgba(34,211,238,0.1)]" : "border-white/10 bg-white/[0.035]"}`}>
                ...
                <Link href={planId === "enterprise" ? "/demo" : `/signup?plan=${planId}`} className="mt-auto rounded-xl bg-white px-4 py-3 text-center text-sm font-bold text-black transition hover:bg-cyan-100">{copy.cta}</Link>
              </article>
            );
          })}
        </section>
