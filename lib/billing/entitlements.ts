...
export type Capability =
  ...
  | "customer_ops.core" | "customer_ops.inbox" | "customer_ops.customers" | "customer_ops.leads" | "customer_ops.conversations" | "customer_ops.followups" | "customer_ops.handoffs" | "customer_ops.appointments" | "customer_ops.tickets"
...
const legacyFeatureCapabilities: Record<BillingFeature, Capability[]> = {
...
export async function getBusinessEntitlements(businessId: string): Promise<BusinessEntitlements> {
  ...
  const limits: EntitlementLimits = { max_ai_employees: plan.employeeLimit, max_human_users: null, max_businesses: plan.id === "enterprise" ? null : 1, max_branches: plan.id === "starter" ? 1 : plan.id === "growth" ? 3 : null, max_knowledge_sources: plan.id === "starter" ? 10 : null, max_automations: plan.automationLimit, max_api_keys: plan.id === "enterprise" ? null : 0, max_monthly_conversations: null, max_storage_mb: null, includedVoiceMinutes: plan.includedVoiceMinutes };
...
export function hasCapability(entitlements: BusinessEntitlements, capability: Capability) { return entitlements.capabilities.includes(capability); }
