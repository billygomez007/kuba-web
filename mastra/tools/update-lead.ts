...
export async function performUpdateLead(businessId: string, employeeId: string, { leadId, leadName, name, email, phone, source, stage }: z.infer<typeof updateLeadInput>) {
  ...
  await createAuditLog({ businessId, userId: null, action: "ai.sales.update_lead", resource: "lead", resourceId: targetLeadId, description: `Kuba Sales updated lead "${updatedLead[0].name}".`, metadata: { ...updates, employeeId } });
...
export const updateLeadTool = createTool({
  ...
  execute: async (input, { requestContext }) => {
    ...
    const decision = await checkAIEmployeeAuthority({ businessId, employeeId, action: "update_lead" });
    if (!decision.ok) {
      if (decision.reason === "requires_approval") {
        const approvalId = await fileActionApproval({ businessId, employeeId, action: "update_lead", payload: input });
    ...
    return performUpdateLead(businessId, employeeId, input);
  },
});
