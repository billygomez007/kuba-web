 ...
 * database bootstrapped here starts at the legitimate 0038 baseline, after
 * which normal generated migrations can continue.
 *
...
if (!baseline || baseline.folderMillis !== 1787700000000) {
  throw new Error("Refusing clean bootstrap: repository baseline is not 0038_current_live_baseline.");
}
...
console.log(JSON.stringify({ databaseType: "local-file", applicationTableCount: Number(tables.rows[0].count) - 1, migrationCount: 1, baseline: "0038_current_live_baseline" }));
client.close();
