...
test("lib/auth.ts wires sendResetPassword to the real password-reset email template, not left unconfigured", async () => {
  ...
  // Resend is resolved lazily via getResend() (lib/email/resend.ts), not a
  // module-load-time `resend` import, so it can't throw on import when no
  // API key is configured — see the callback actually sending the email.
  assert.match(emailAndPasswordBlock, /getResend\(\)\.emails\.send/);
});
